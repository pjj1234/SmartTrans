...
import { insertReport } from '../db/reports.repo'
...
export async function* runPipeline(
  ...
  try {
    ...
    const scene = await recognizeScene(images, description)
    ...
    const severity = await assessSeverity(scene, description)
    ...
    const { analysis: liability } = await analyzeLiability(scene, severity, description)
    ...
    const report = await generateReport({ scene, severity, liability, description })
